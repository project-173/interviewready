    ...
    def orchestrate(self, request: ChatRequest, context: SessionContext) -> AgentResponse:
        ...
        with langfuse.trace(
            ...
            if intent not in {"RESUME_CRITIC", "CONTENT_STRENGTH", "ALIGNMENT", "INTERVIEW_COACH"}:
                trace.update(output={"error": f"Unsupported intent: {intent}"})
                logger.log_agent_error("OrchestrationAgent", ValueError(f"Unsupported intent: {intent}"), session_id)
            ...
            try:
                ...
                trace.update(output={"normalization_result": normalization_result, "agent_sequence": agent_sequence})
